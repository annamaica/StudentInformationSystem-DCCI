...
if(isset($_POST['submit'])){
    $update_id = $_GET['edit'];
    $newemployeenum = $_SESSION['employeenum'];
    ...
    if($newemployeenum && $newlastname && $newfirstname || $newmiddlename && $newstreet && $newbarangay && $newmunicipality && $newdistrict && $newprovince && $newcontactnum && $newemailadd && $newgender && $newdob && $newage && $newcivilstatus)
    {
      $update_query = "UPDATE tblemployee set Employee_Number='$newemployeenum',Last_Name='$newlastname',First_Name='$newfirstname',Middle_Name='$newmiddlename',Street='$newstreet',Barangay='$newbarangay',Municipality='$newmunicipality',District='$newdistrict',Province='$newprovince',Contact_Number='$newcontactnum',Email_Address='$newemailadd',Gender='$newgender',Date_of_Birth='$newdob',Age='$newage',Civil_Status='$newcivilstatus' where Employee_Number='$update_id'";

        if(mysql_query($update_query))
        {
          echo "<script>alert('Employee Updated!');
          window.location.replace('view_faculty.php?faculty=$update_id');</script>";
        }
    }
    else
    {

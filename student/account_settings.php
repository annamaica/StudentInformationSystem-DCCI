...
<?php include('includes/navigation.php'); 
	$student_id = $_SESSION['studentnum'];
	$query = mysql_query("SELECT * FROM tbluser WHERE User_ID = '$student_id'");
				...
				if(!empty($_POST["newpassword"]) && !empty($_POST['confirmpassword'])){
					if($_POST["newpassword"]===$_POST["confirmpassword"]){
						if(strlen($_POST['confirmpassword'])>3){
							...
							$insert = mysql_query("UPDATE tbluser SET Pword='$passhold' WHERE User_ID='$student_id'");
